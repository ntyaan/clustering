...
int main(void){
  ...
  ifs >> data_number;
  ifs >> data_dimension;
  OCS_BFCS test(data_dimension, data_number, centers_number, 1.001, 1000);
  //読み込んだデータを格納
  ...
  //データ欠損
  test.set_missing_values(0, 2);
  //初期クラスタ中心
  test.initialize_centers(0);
  //更新式ループ回数
  test.iterates()=0;
  while(1){
    test.revise_data();
  ...
  for(int k=0;k<data_number;k++){
    ...
    for(int ell=0;ell<data_dimension;ell++)
      if(test.incompleteindex(k,ell)==1)
	L=1;
    if(L==1)
      std::cout<<test.data(k)<<std::endl
	       <<test.correctdata(k)<<std::endl<<std::endl;
  }
  */
  //}
  //確認
