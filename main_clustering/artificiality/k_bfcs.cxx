...
int main(void){
  ...
  std::string filenameData 
    ("data/clustering/test/sparse_circle.txt");
  //読み込むデータファイルの正解の帰属度ファイル
  std::string filenameCorrectCrispMembership
    ("data/clustering/test/circle_correct.txt");
  //データの読み込み
  ...
  ifs >> data_number;
  ifs >> data_dimension;
  K_BFCS test(data_dimension, data_number, centers_number, 1.0/1.1, 32.0);
  //読み込んだデータを格納
  ...
  for(int i=0;i<test.centers_number();i++){
    ...
    for(int k=0;k<test.data_number();k++){
      ...
      if(test.correctCrispMembership(i, k)==1)
	C[i]++;
    }
  }
  //時間計測
  ...
  while(1){
    ...
    //dissimilarities
    double diff_d=max_norm(test.tmp_dissimilarities()-test.dissimilarities());
    //全体の収束具合
    double diff=diff_u+diff_p+diff_d;
    //diffの出力
    std::cout<<"diff:"<<diff<<std::endl;
    //収束したらクラスタリング終了
    if(diff<DIFF_FOR_STOP && test.iterates()> 2)break;
