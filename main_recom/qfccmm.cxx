...
int main(void){
  ...
  for(int clusters_number=2;clusters_number<=7;clusters_number++){
    ...
    for(double m=1.0001;m<=1.001;m+=0.0003){
      //for(double m=1.0+1.0E-15;m>=1.0+1.0E-100;m=(m-1.0)/10.0+1.0){
      //パラメータlambdaでループ
      for(double l=10;l<=40;l+=10){
	//for(double l=0.1;l<=0.1;l*=2){
	//パラメータtでループ
	//for(double t=1.0E-7;t>=1.0E-100;t/=10.0){
	for(double t=1.0E-6;t<=1.0E-4;t*=10){
	  //for(double t=1.0E-5;t<=1.0E-5;t*=10){
	  //時間計測
	  auto start=std::chrono::system_clock::now();
	  //SparseAfccmdクラスの生成
	  QFCCMM test(item_number, user_number
		      , clusters_number, m, l, t);
	  //有効数字5桁で表示
	  std::ostringstream oss1,oss2,oss3;
	  oss1<<std::setprecision(15)<<m;
	  oss2<<std::setprecision(15)<<l;
	  oss3<<std::setprecision(15)<<t;
