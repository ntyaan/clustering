...
\subsection{QPCS}
\begin{frame}\frametitle{QPCS}%\scriptsizeTsallis Entropy-regularized q-divergence based Fuzzy $c$-means with variables controlling cluster sizes)}
  \begin{block}{optimization problem}
    $\underset{u,v}{\text{minimize}}$
...
\end{frame}
%\subsection{qFCCM(仮)}
%\begin{frame}\frametitle{qFCCM(仮)}
%  \begin{block}{optimization problem}
%    $\underset{u,w,\pi}{\text{maximize}}~J_{qFCCM}$\\
%    $J_{qFCCM}=\sum_{i=1}^C\sum_{k=1}^N(\pi_i)^{1-m}(u_{i,k})^m(\frac{1}{(m-1)(m-2)}\sum_{\ell=1}^M(x_{k,\ell})^{2-m}(1-(w_{i,\ell})^{m-1}))$\\$+\frac{\lambda^{-1}}{1-m}\sum_{i=1}^C\sum_{k=1}^N (\pi_i)^{1-m}(u_{i,k})^m$\centering 	%\raggedleft
%  \end{block}
%  $s_{i,k}=\frac{1}{(m-1)(m-2)}\sum_{l=1}^M(x_{k,\ell})^{2-m}(1-(w_{i,\ell})^{m-1})$\centering\\
%  \begin{align*}
%    u_{i,k}&=\frac{\pi_i(1+\lambda(1-m)s_{i,k})^{\frac{1}{1-m}}}{\sum_{j=1}^C\pi_{j}(1+\lambda(1-m)s_{j,k})^{\frac{1}{1-m}}},
%    w_{i,\ell}=\frac{(\sum_{k=1}^N (u_{i,k})^m (x_{k,\ell})^{2-m})^{\frac{1}{2-m}}}{\sum_{r=1}^M (\sum_{k=1}^N (u_{i,k})^m (x_{k,r})^{2-m})^{\frac{1}{2-m}}},\\
%    \pi_i&=\frac{(\sum_{k=1}^N (u_{i,k})^m(1+\lambda(1-m) s_{i,k}))^{\frac{1}{m}}}{\sum_{j=1}^C (\sum_{k=1}^N (u_{j,k})^m(1+\lambda(1-m) s_{j,k}))^{\frac{1}{m}}}.\\
%  \end{align*}
%\end{frame}
